...
function loadPlayers (p, mapID) {
  ...
  var wsuri = 'ws://' + WSHost + '/ws/getPlayer'
  ...
  p.forEach(function (e, i) {
    ...
    window.onbeforeunload = function () {
      playersSockets[i].onclose = function () {} // disable onclose handler first
      playersSockets[i].close()
    }
    ...
    playersSockets[i].onmessage = function (e) {
      if (e.data.indexOf(':')) {
        ...
        data = JSON.parse(res.substring(index + 1, res.length))
